...
# Usage: (from repository root, run) ./experiments/monkeys-playroom-numeric-domain/run_monkeysNumeric07_experiments.sh <num_trials> <optional scope_true_or_false>
...
# Example: ./experiments/monkeys-playroom-numeric-domain/run_monkeysNumeric07_experiments.sh 5 scope_true
TIMEFORMAT=%R
user_arg="$1"
...
until [ $COUNTER -lt 1 ]; do
    echo RUN_NUMBER $COUNTER
    exec 3>&1 4>&2
    foo=$( { time enhsp-2020 -o examples/multi_monkeys_playroom/multi_monkeys_playroom.pddl -f examples/multi_monkeys_playroom/prob07.pddl -planner opt-hlm  1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    enhsp_runtimes+=("$foo")
    let COUNTER-=1
done
...
until [ $COUNTER -lt 1 ]; do
    echo RUN_NUMBER $COUNTER
    exec 3>&1 4>&2
    foo=$( { time enhsp-2020 -o examples/multi_monkeys_playroom/multi_monkeys_playroom_scoped_prob07.pddl -f examples/multi_monkeys_playroom/prob07_scoped.pddl -planner opt-hlm 1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    senhsp_runtimes+=("$foo")
    let COUNTER-=1
done

...
then
    ...
    until [ $COUNTER -lt 1 ]; do
        ...
        exec 3>&1 4>&2
        foo=$( { time python scoping/scope_and_writeback_pddl.py --domain examples/multi_monkeys_playroom/multi_monkeys_playroom.pddl --prob examples/multi_monkeys_playroom/prob01.pddl 1>&3 2>&4; } 2>&1 )  # Captures time only.
        exec 3>&- 4>&-
        sruntimes+=("$foo")
        let COUNTER-=1
    done

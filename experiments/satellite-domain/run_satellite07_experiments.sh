...
until [ $COUNTER -lt 1 ]; do
    ...
    exec 3>&1 4>&2
    foo=$( { time python scoping/downward_translate/translate_and_scope.py examples/IPC_domains_propositional/satellite/domain.pddl examples/IPC_domains_propositional/satellite/prob07.pddl --sas-file satellite07.sas 1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    truntimes+=("$foo")
    let COUNTER-=1
done
...
until [ $COUNTER -lt 1 ]; do
    echo RUN_NUMBER $COUNTER
    exec 3>&1 4>&2
    foo=$( { time python scoping/downward_translate/translate_and_scope.py examples/IPC_domains_propositional/satellite/domain.pddl examples/IPC_domains_propositional/satellite/prob07.pddl --sas-file satellite07.sas --scope True 1>&3 2>&4; } 2>&1 )  # Captures time only.
    exec 3>&- 4>&-
    sruntimes+=("$foo")
    let COUNTER-=1
done
